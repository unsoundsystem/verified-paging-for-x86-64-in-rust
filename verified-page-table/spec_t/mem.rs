...
pub open spec fn word_index_spec(addr: nat) -> nat
    recommends aligned(addr as nat, 8)
{
...
impl PageTableMemory {
    ...
    pub fn alloc_page(&mut self) -> (r: MemRegionExec)
        ...
        ensures
            ...
            aligned(r@.base as nat, PAGE_SIZE as nat),
    ...
    pub fn write(&mut self, paddr: usize, region: Ghost<MemRegion>, value: u64)
        requires
            ...
            word_index_spec(sub(paddr as nat, region@.base) as nat) < 512,
        ensures
            self.region_view(region@) === old(self).region_view(region@).update(word_index_spec(sub(paddr as nat, region@.base) as nat) as int, value),
    ...
    pub open spec fn spec_read(self, paddr: nat, region: MemRegion) -> (res: u64) {
        self.region_view(region)[word_index_spec(sub(paddr as nat, region.base) as nat) as int]
    }
}
